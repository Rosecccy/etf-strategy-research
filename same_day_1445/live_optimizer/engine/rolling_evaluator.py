...
def compute_metrics(rows: list[dict[str, Any]], accounting: str) -> EvaluationMetrics:
    ...
    return EvaluationMetrics(
        ...
        trigger_retention=(trades / opportunities) if opportunities else 0.0,
        win_rate=win_rate,
...
def _row_date(row: dict[str, Any]) -> date:
    value = str(row.get("decision_at") or row.get("entry_date") or "")
...
def evaluate_candidate_windows(candidate_rows: list[dict[str, Any]], baseline_rows: list[dict[str, Any]], windows: list[dict[str, Any]], accounting: str) -> dict[str, Any]:
    ...
    for window in windows:
        ...
        c_rows = [row for row in candidate_rows if start <= _row_date(row) <= end]
        b_rows = [row for row in baseline_rows if start <= _row_date(row) <= end]
        ...
        improved = win_delta >= 0 and mean_delta >= 0 and final_delta >= 0
        window_results.append({
            "name": str(window["name"]),
            "candidate": cm.to_dict(),
            "baseline": bm.to_dict(),
