...
OUT = ROOT / "fit" / "s_robust_plateau"
MIN_TRAIN_TRADES = 12
MIN_TRAIN_TRIGGERS = 2


def parameters(rule: str) -> tuple[float, float, int]:
    match = re.search(r":a([+-]\d+\.\d+):b([+-]\d+\.\d+):h(\d+)$", rule)
    if not match:
...
def choose_plateau_rule(
    ...
    for name, profile in profiles.items():
        ...
        if (
            ...
            arm, floor, hold = parameters(name)
            candidates.append(
                {
                    "rule": name,
                    "arm": arm,
    ...
    plateau = [item for item in candidates if item["ratio"] >= cutoff]
    median_arm = float(np.median([item["arm"] for item in plateau]))
    median_floor = float(np.median([item["floor"] for item in plateau]))
    median_hold = float(np.median([item["hold"] for item in plateau]))
    for item in plateau:
        item["center_distance"] = (
            abs(item["arm"] - median_arm) / 0.005
            + abs(item["floor"] - median_floor) / 0.005
            + abs(item["hold"] - median_hold) / 3.0
        )
    winner = min(plateau, key=lambda item: (item["center_distance"], -item["ratio"]))
...
def main() -> None:
    ...
    summary = {
        "method": "annual expanding-window near-optimal three-parameter plateau center",
